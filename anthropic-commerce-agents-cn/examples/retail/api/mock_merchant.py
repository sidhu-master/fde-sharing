...
class MockRetailMerchant(MerchantBackend):
    ...
    def home_insights(self, limit: int = 3) -> list[dict[str, Any]]:
        ...
        if worst is not None:
            ...
            pace = f"，近 30 天售出 {sold} 件" if sold else ""
            insights.append(
                {
                    "insight_id": f"return-rate-{product.product_id}",
                    "kind": "returns",
                    "headline": f"{product.title} 的退货率达到 {rate:.0f}%",
                    "detail": f"退货率为 {rate:.0f}%{pace}；建议补货前检查商品页面内容。",
                    "prompt": f"{product.title}（{product.product_id}）的退货率为 "
                    f"{rate:.0f}%。可能是什么原因，应该做哪些调整？",
                }
            )
                ...
                insights.append(
                    {
                        ...
                        "kind": "trend",
                        "headline": f"儿童房品类销售额环比{direction} {abs(change):.0f}%",
                        "detail": f"儿童房品类本周 ${current:,.0f}，上周 ${prior:,.0f}。",
                        "prompt": f"儿童房品类销售额环比{direction} {abs(change):.0f}%。"
                        "主要驱动因素是什么，哪些商品有缺货风险？",
                    }
                )
                ...
                insights.append(
                    {
                        ...
                        "kind": "campaign",
                        "headline": "当前营销活动的投入回报差异明显",
                        "detail": f"{best.name} 每投入 $1 回报 ${best_roas:.2f}；"
                        f"{worst_campaign.name} 回报 ${worst_roas:.2f}。",
                        "prompt": f"{best.name} 每投入 $1 回报 ${best_roas:.2f}，"
                        f"{worst_campaign.name} 为 ${worst_roas:.2f}。是否应调整预算？",
                    }
                )
