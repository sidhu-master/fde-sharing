...
export default function PortalPage() {
  ...
  const nav = useMemo<PortalNavItem<PortalView>[]>(() => {
    ...
    return [
      ...
      { id: "catalog", label: "商品", icon: "tag" },
      { id: "orders", label: "订单", icon: "inbox", attention: alerts?.order_issues || null },
      {
        id: "inventory",
        label: "库存",
        icon: "box",
  ...
  return (
    <>
      <PortalShell
        brand={{ mark: <StoreMark />, name: "ACME", detail: "商家工作台" }}
        nav={nav}
        view={view}
        onViewChange={setView}
        operator={{ name: session.operator ?? "运营人员", role: "店铺经理" }}
        assistantLabel="智能助手"
