...
function metricLabel(metric: string): string {
  const labels: Record<string, string> = { sales: "销售额", orders: "订单数", average_order_value: "平均客单价", revenue: "收入", spend: "支出", conversion_rate: "转化率", return_rate: "退货率", click_through_rate: "点击率" };
...
export default function MetricsCard({ payload }: { payload: MetricsPayload }) {
  ...
  return (
    <GenCard>
      <GenCardHeader title={payload.title ?? "经营表现"} aside={payload.period ? formatPeriodLabel(payload.period) : null} />
