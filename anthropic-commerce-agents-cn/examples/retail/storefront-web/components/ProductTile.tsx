...
export function DeliveryPromise({
  ...
  const match = /^Get it by ([A-Z][a-z]{2}), ([A-Z][a-z]{2}) (\d{1,2})$/.exec(promise);
  const months: Record<string, number> = { Jan: 1, Feb: 2, Mar: 3, Apr: 4, May: 5, Jun: 6, Jul: 7, Aug: 8, Sep: 9, Oct: 10, Nov: 11, Dec: 12 };
  const weekdays: Record<string, string> = { Mon: "一", Tue: "二", Wed: "三", Thu: "四", Fri: "五", Sat: "六", Sun: "日" };
  const localized = match ? `预计 ${months[match[2]]}月${match[3]}日（周${weekdays[match[1]]}）送达` : promise;
  return (
    <div className={`text-[11px] font-medium text-(--ok) ${className}`}>{localized}</div>
  );
}
