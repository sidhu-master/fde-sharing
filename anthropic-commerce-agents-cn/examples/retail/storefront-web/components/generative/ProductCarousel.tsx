  ...
  return (
    <div data-review-aspects className="mt-2">
      <div className="text-[11px] font-semibold uppercase tracking-wide text-(--ink-soft)">
        基于 {synthesis.review_count.toLocaleString()} 条顾客评价
      </div>
          ...
          >
            <div className="flex items-baseline gap-1.5 text-[13px]">
              ...
              <span className="text-[11px] text-(--ink-soft)">
                {aspect.mentions.toLocaleString()} 次提及
              </span>
            </div>
...
function VariantList({ family, variants }: { family: Product; variants: Product[] }) {
  ...
  return (
    <ul className="mt-2 flex flex-wrap gap-1.5" aria-label="商品规格">
        ...
        return (
          <li key={variant.product_id}>
            <button
              type="button"
              disabled={!available}
              onClick={() => ask(`将 ${family.title} 的 ${label} 规格（${variant.product_id}）加入购物车。`)}
