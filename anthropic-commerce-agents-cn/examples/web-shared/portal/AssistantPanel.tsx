    ...
    <div className="flex h-full w-full flex-col border-l border-(--line) bg-(--card)">
      <div className="flex items-center gap-2.5 border-b border-(--line) py-3 pl-4 pr-2.5">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="truncate text-[11.5px] text-(--ink-soft)">{copy.approvalNote ?? "You approve every change"}</div>
        </div>
        <ActivityButton streaming={chat.streaming} newMemoryCount={newMemoryCount} onClick={onOpenActivity} label={copy.activityLabel} />
        {onToggleFullscreen ? (
          <IconButton
            icon={fullscreen ? "collapse" : "expand"}
            label={fullscreen ? (copy.exitFullScreenLabel ?? "Exit full screen") : (copy.fullScreenLabel ?? "Full screen")}
            onClick={onToggleFullscreen}
            className="hidden lg:grid"
          />
        ) : null}
        <IconButton icon="x" label={copy.hideLabel ?? "Hide assistant"} onClick={onClose} />
      </div>
      ...
      <div className="border-t border-(--line) p-3">
        <Composer
          ...
          variant="field"
          className={column}
          busyPlaceholder={copy.activityLabel ? "处理中…" : undefined}
          sendLabel={copy.activityLabel ? "发送" : undefined}
        />
      </div>
    </div>
  );
