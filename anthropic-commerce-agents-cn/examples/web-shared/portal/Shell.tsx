    ...
    <div className="flex h-dvh bg-(--ground) text-(--ink)">
      <aside className="hidden w-16 shrink-0 flex-col border-r border-(--line) bg-(--chrome) px-2 py-3.5 lg:flex xl:w-[220px] xl:px-3">
        ...
        <button
          ...
          aria-label={assistantOpen ? `隐藏${assistantLabel}` : `显示${assistantLabel}`}
          title={assistantOpen ? `隐藏${assistantLabel}` : `显示${assistantLabel}`}
        ...
        >
          ...
          <span className="hidden flex-1 xl:block">{assistantLabel}</span>
          {assistantBusy ? (
