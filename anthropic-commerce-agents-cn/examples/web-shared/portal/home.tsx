...
export function QueueOverflow({ hidden, link, label }: { hidden: number; link?: { label: string; onClick: () => void }; label?: string }) {
  if (hidden <= 0) return null;
  return (
    <div className="flex items-center gap-2 border-t border-(--line) px-[18px] py-2.5 text-[12.5px] text-(--ink-soft)">
      <span>{label ?? `${plural(hidden, "more item")} in the queue`}</span>
