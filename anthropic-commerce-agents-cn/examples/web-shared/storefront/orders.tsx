...
export function estimateOf(order: Order, locale = "en-US"): { date: string; note: string | null } | null {
  ...
  return {
    ...
    note: match ? match[2].replace(ISO_DAY, (value) => formatDayMonth(value, locale)) : null,
  };
}
...
export interface OrderNouns {
  ...
  cardTitle: string;
  /** Optional localized label for the home card action. */
  allLabel?: string;
  locale?: string;
...
function When({ order, nouns }: { order: Order; nouns: OrderNouns }) {
  ...
  return (
    <span className={order.status === "delayed" ? "font-semibold text-(--warn)" : ""} title={estimate.note ?? undefined}>
      {order.status === "delayed" ? (nouns.delayedVerb ?? "Expected") : nouns.openVerb} {estimate.date}
    </span>
  );
}
  ...
  return (
    <li className="flex items-center gap-3 border-t border-(--line) px-[18px] py-3 first:border-t-0">
      ...
      <div className="min-w-0 flex-1">
        <div className="truncate text-[14px] font-semibold text-(--ink)">{orderTitle(order, nouns)}</div>
...
export function ArrivingPanel({
  ...
  return (
    <Panel
      ...
      subtitle={open.length ? (nouns.countLabel?.(orders.length) ?? plural(orders.length, nouns.one)) : nouns.noneOpen}
      action={onSeeAll ? <MoreLink label={nouns.allLabel ?? `All ${nouns.title.toLowerCase()}`} onClick={onSeeAll} /> : null}
    >
      <ul>
        {shown.map((order) => (
